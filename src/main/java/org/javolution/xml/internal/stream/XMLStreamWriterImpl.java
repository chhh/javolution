...
public final class XMLStreamWriterImpl implements XMLStreamWriter {
    ...
    private final void write(Object csq, int start, int length,
        ...
        if (_index + length <= BUFFER_LENGTH) { // Enough buffer space.
            ...
            } else if (csq instanceof org.javolution.text.Text) {
                ((org.javolution.text.Text) csq).getChars(start, start + length,
                        _buffer, _index);
            } else if (csq instanceof org.javolution.text.TextBuilder) {
                ((org.javolution.text.TextBuilder) csq).getChars(start, start
                        + length, _buffer, _index);
            } else if (csq instanceof org.javolution.text.CharArray) {
                ((org.javolution.text.CharArray) csq).getChars(start, start
                        + length, _buffer, _index);
            } else {
