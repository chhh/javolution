...
/**
 * A context for executing and chaining calculations in parallel on computing devices such as CPUs/GPUs. 
 * ComputeContext keeps its {@link Buffer buffers} in device memory unless a buffer is {@link Buffer#export exported} 
 ...
 * a {@link ComputeContext} (already intrinsically parallel) is ill-advised (the reverse is fine though).         
 * 
 * @author  <jean-marie@dautelle.com>
 * @version 7.0, March 31, 2017
 * @see [Wikipedia: OpenCL](http://en.wikipedia.org/wiki/OpenCL)
 */
public abstract class ComputeContext extends AbstractContext {
    ...
    public interface Buffer extends Local {
         ...
         * @throws UnsupportedOperationException if {@link ComputeContext#DOUBLE_PRECISION_REQUIRED} is 
    ...
    /**
     * Enters the scope of a local computing context; all the resources (kernels, buffers) allocated 
     * while in this context scope are released upon {@link AbstractContext#exit() exit}.
